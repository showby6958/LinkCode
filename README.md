...
```
                         ...
                         │              Client (Browser)            │
                         └──────┬─────────────┬────────────────────┘
                                │ REST / WS    │ Binary WS (Y.js)
              ┌─────────────────▼──┐      ┌───▼──────────────────┐
              │    chat-service    │      │  interview-service   │
              │      :8082         │      │      :8083            │
              │  STOMP + SockJS    │      │  코드동기화/면접관리  │
              │  Transactional     │      │  Binary WebSocket    │
              │  Outbox Pattern    │      └──────────┬───────────┘
              └────────┬───────────┘                 │
                       │ Kafka (CHAT_MESSAGE)         │ Kafka (interview-ended)
              ┌────────▼───────────┐      ┌──────────▼──────────┐
              │ chat-consumer-svc  │      │   AI 분석 Consumer  │
              │  (dynamic port)    │      │  (interview-service) │
              │  @RetryableTopic   │      └─────────────────────┘
              │  (5회 지수 백오프) │
              └────────┬───────────┘
                       │ Redis (최신 100건)
              ┌────────▼──────────────────────────────────────┐
              │                  Redis :6379                   │
              │  chat:room:{id}  │  interview:room:{id}:yjs   │
              │  (TTL 1h, max100)│  (Y.js delta history)      │
              └───────────────────────────────────────────────┘

              ┌──────────────────────────────────────────────┐
              │              auth-service :8081              │
              │  Google / Kakao / Naver OAuth2               │
              │  JWT AccessToken(30m) + RefreshToken(4d)     │
              ...
              │  KafkaEventPublisher / ChatMessageDto        │
              └──────────────────────────────────────────────┘
```
...
```
신규 유저 접속 시 → Redis delta 히스토리 전체 스트리밍 → 현재 상태 복원
기존 유저 편집 시 → delta broadcast → Redis append → 영속 보장
면접 종료 시      → Redis snapshot 수거 → Kafka 발행 (AI 분석) → Redis 정리
```
...
| Auth | OAuth2 Client (Google/Kakao/Naver), JJWT 0.12 |
| Message Queue | Apache Kafka (Transactional Outbox, RetryableTopic, DLT) |
...
├── chat-service/           # :8082 — WebSocket 채팅 + Outbox
├── chat-consumer-service/  # dynamic — Kafka → Redis consumer
├── interview-service/      # :8083 — 협업 코딩 면접 + AI 분석 파이프라인
