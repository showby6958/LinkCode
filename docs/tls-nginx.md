...
git clone https://github.com/showby6958/LinkCode.git          # main 브랜치(기본)
...
- `.env` — `OAUTH_REDIRECT_BASE` / `CORS_ORIGINS` / `FRONTEND_URL` 를 `https://<도메인>` 으로 (아래 3번)
...
FRONTEND_URL=https://linkcoder.duckdns.org       # 로그인 후 리다이렉트·초대링크 주소
```
나머지(DB/JWT/OAuth/Slack/GEMINI, 인프라 호스트)는 로컬과 동일. 단 비밀값은 공개 서버용으로 강하게.
> `application.yml` 은 이제 `${VAR}` 로만 참조하고 git으로 커밋되므로, 비밀값은 이 `.env` 에만 두면 된다(수동 scp 불필요).
...
git pull && $DC up -d --build            # 수동 배포 (프론트 변경도 재빌드로 반영)
docker system prune -af                  # 디스크 정리
```
> **자동 배포(CI/CD)**: `main` 에 push하면 GitHub Actions(`.github/workflows/deploy.yml`)가
> 서버에서 `deploy.sh`(3개 레포 pull + `up -d --build`)를 자동 실행한다. 즉 평소엔 `git push` 만 하면 된다.
> nginx는 `api.conf` 의 `resolver` 로 업스트림 IP를 런타임 재해석하므로, 재배포 후 **nginx 재시작이 필요 없다**.
